...
SQL_STATEMENTS = """
create table tgt_table1 as select col11,col12,col13,col14,col15 from orc.src_table1;
create table tgt_table4 as select col11,col12,col13,col14,col15 from orc.src_table2;
create table tgt_table2 as select col11 as col21,col12 as col22,col13 as col23,col14 as col24,col15 as col25 from tgt_table1;
create table tgt_table3 as select COALESCE(B.col21,A.col11) as col31,case when C.col11=A.col11 THEN 1 ELSE 0 END as  col32,A.col23 as col33,A.col24 as col34,A.col25 as col35
 from tgt_table2 B
join tgt_table1 A on A.col11=B.col21
join tgt_table4 C on C.col11=A.col11;
create table tgt_table5 as select tmp.* from (select col11 as col51,col12 as col52,col13 as col53,col14 as col54,col15 as col55 
from tgt_table1 where col11>100) tmp;

insert into orc.tgt_final as select * from tgt_table3 join tgt_table5 on tgt_table3.col31=tgt_table5.col51;
"""
    ...
    try:
        ...
        if from_clause and getattr(from_clause, "this", None):
            ...
            if isinstance(tbl, exp.Subquery):
                ...
                if getattr(tbl, "this", None):
                    table_name_clean = tbl.this.sql()
                else:
                    table_name_clean = tbl.sql()
        ...
        for join in select_exp.args.get("joins") or []:
            if getattr(join, "this", None):
                ...
                if isinstance(jt, exp.Subquery):
                    ...
                    if getattr(jt, "this", None):
                        table_name_clean = jt.this.sql()
                    else:
                        table_name_clean = jt.sql()
...
def expand_select_star_for_target(table_defs: Dict[str, Dict[str, Any]], target: str, col_rename_map: Optional[Dict[str, str]] = None) -> Dict[str, List[Dict[str, Any]]]:
    ...
    try:
        ...
        return tgt_col_lineage
    except Exception as e:
        print(f"Warning: Error in expand_select_star_for_target: {e}")
        return {}


def build_final_lineage(table_defs: Dict[str, Dict[str, Any]], target: str = None, default_schema: str = "default") -> Dict[str, Any]:
    ...
    try:
        ...
        def add_schema_prefix(table_name: str, default_schema: str) -> str:
            ...
            for src in src_list:
                                ...
                                for candidate, td in table_defs.items():
                                    if cname in td.get("column_lineage", {}):
                                        ...
                                        if bases:
                                            break
                            for bt, bc in bases:
                                if bt and bc and str(bt).upper() != "N/A":
                                    bt_qualified = add_schema_prefix(bt, default_schema)
                ...
                if st and sc:
                    if st in table_defs:
                        ...
                        if str(st).upper() != "N/A":
                            st_qualified = add_schema_prefix(st, default_schema)
        ...
        source_tables_qualified = [add_schema_prefix(t, default_schema) for t in tgt_def.get("source_tables", [])]
...
def generate_lineage(sql: str, target_table: str = None, default_schema: str = "default") -> Dict[str, Any]:
    """
...
def main():
    # Works with any SQL - auto-detects target table and schema
    result = generate_lineage(SQL_STATEMENTS, default_schema="orc")
